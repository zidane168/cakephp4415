...
<?= $this->Html->script('CakeAdminLTE/pages/multi_videos_upload_container.js?v=' . date('U'), array('inline' => false)); ?>

<script>
    $(document).ready(function() {
        MULTI_VIDEO_UPLOADS.limit_size = <?php echo $limit_size; ?>;
        MULTI_VIDEO_UPLOADS.lst_material = [];
        MULTI_VIDEO_UPLOADS.lst_material_extension_allowed = <?= isset($accept_file) && !empty($accept_file) ?  json_encode($accept_file) : json_encode($default_accept_file) ?>;
        MULTI_VIDEO_UPLOADS.common();
        MULTI_VIDEO_UPLOADS.add();
