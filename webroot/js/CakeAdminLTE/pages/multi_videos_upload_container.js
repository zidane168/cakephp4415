var MULTI_VIDEO_UPLOADS = {
  ...
  validation_file: function (file) {
    ...
    if (
      MULTI_VIDEO_UPLOADS.lst_material_extension_allowed.indexOf(
        "." + f_extension
      ) == -1
    ) {
      return false;
    }
    if (file.size > MULTI_VIDEO_UPLOADS.limit_size * 1024 * 1024) {
  ...
  remove_material: function (index) {
    MULTI_VIDEO_UPLOADS.lst_material.splice(index, 1);
    MULTI_VIDEO_UPLOADS.get_file_preview();

    // updatefileLists (input file)
    MULTI_VIDEO_UPLOADS.update_file_lists();
  },
  ...
  get_file_preview: function () {
    var files = MULTI_VIDEO_UPLOADS.lst_material;
    var preview_img = "";
    for (var i = 0; i < files.length; i++) {
      ...
      preview_img += MULTI_VIDEO_UPLOADS.bytes_to_size(files[i].size);
      preview_img += '      <span class="seperate"></span>';
      preview_img += "(" + f_extension + ")";
      preview_img +=
        '  </p>  <span class="fa fa-times remove-file pointer" onclick="MULTI_VIDEO_UPLOADS.remove_material(' +
        i +
        ')"></span>';
      preview_img += "</li>";
    }
  ...
  toggle_current_material: function (material_id) {
    var index = MULTI_VIDEO_UPLOADS.lst_remove_material.indexOf(material_id);

    if (index != -1) {
      MULTI_VIDEO_UPLOADS.lst_remove_material.splice(index, 1);
    } else {
      MULTI_VIDEO_UPLOADS.lst_remove_material.push(material_id);
    }

    $("#material-" + material_id + " .remove-file").toggleClass("hidden");
    $("#material-" + material_id + "").toggleClass("removed");
    $("#remove_image").val(MULTI_VIDEO_UPLOADS.lst_remove_material);
  ...
  update_file_lists: function () {
    ...
    MULTI_VIDEO_UPLOADS.lst_material.forEach((file) => {
  ...
  add: function () {
    ...
    $(".import-file-area").on("drop", function (e) {
      ...
      for (let i = 0; i < files.length; i++) {
        if (MULTI_VIDEO_UPLOADS.validation_file(files[i])) {
          MULTI_VIDEO_UPLOADS.lst_material.push(files[i]);
          MULTI_VIDEO_UPLOADS.get_file_preview();
        }
      }
  ...
  // for edit
  edit: function () {
    let listDataString = MULTI_VIDEO_UPLOADS.lst_edit_material_string;
    ...
    lists.forEach((item, index) => {
      MULTI_VIDEO_UPLOADS.lst_edit_material.push(item);
    });
    MULTI_VIDEO_UPLOADS.get_edit_file_preview();
    ...
    $(".import-file-area").on("drop", function (e) {
      ...
      MULTI_VIDEO_UPLOADS.get_edit_add_file_preview();
      MULTI_VIDEO_UPLOADS.update_file_lists();
    });
  ...
  get_edit_file_preview: function () {
    let preview_img = "";
    let arrFiles = MULTI_VIDEO_UPLOADS.lst_edit_material;
    ...
    for (var i = 0; i < arrFiles.length; i++) {
      ...
      preview_img += '  <p class="file-info text-grey">';
      preview_img += MULTI_VIDEO_UPLOADS.bytes_to_size(arrFiles[i].size);
      preview_img += '      <span class="seperate"></span>';
      preview_img += "(" + f_extension + ")";
      preview_img += "  </p>";
      preview_img +=
        '  <span class="fa fa-times remove-file pointer" onclick="MULTI_VIDEO_UPLOADS.remove_edit_material(' +
        arrFiles[i].id +
        ", " +
        i +
        ')"></span>';
  ...
  get_edit_add_file_preview: function () {
    ...
    arrFiles = MULTI_VIDEO_UPLOADS.lst_material;
    ...
    for (var i = 0; i < arrFiles.length; i++) {
      ...
      preview_img += "  </p>";
      preview_img +=
        '  <span class="fa fa-times remove-file pointer" onclick="MULTI_VIDEO_UPLOADS.remove_add_material(' +
        i +
        ')"></span>';
      preview_img += "</li>";
    }

    console.log(MULTI_VIDEO_UPLOADS.lst_material);
    $("#lst-imported-material").html(preview_img);
  },

  remove_edit_material: function (image_id, id) {
    MULTI_VIDEO_UPLOADS.lst_edit_material.splice(id, 1);
    MULTI_VIDEO_UPLOADS.lst_remove_material.push(image_id);
    $("#remove_image").val(
      JSON.stringify(MULTI_VIDEO_UPLOADS.lst_remove_material)
    );
