...
$(document).ready(function () {
  $(".my-box").each(function (i, v) {
    var box = $(this).children(".box").first(); // Find the body and the footer

    var bf = box.find(".card-body, .box-footer");

    if (!box.hasClass("collapsed-box")) {
      box.addClass("collapsed-box"); // Convert minus into plus

      $(this).children(".fa-minus").removeClass("fa-minus").addClass("fa-plus");
      bf.slideUp();
    } else {
      box.removeClass("collapsed-box"); // Convert plus into minus
  ...
  $("input.chk-all-permission").change(function (event) {
    if ($(this).is(":checked")) {
      $(this).closest("table").find(".chk-permission-id").prop("checked", true); // replace iCheck (jquery 2.2.1) to prop('checked', true) (jquery 3.5.1)
    } else {
  ...
  $(document).on("click", ".fa-minus", function () {
    // get .box parent
    box = $(this).closest(".card"); // change minus => plus and collapsed
    ...
    if (!box.hasClass("collapsed-box")) {
      box.addClass("collapsed-box");
      var plus = box.find(".fa-minus"); // plus.attr("class", "fa fa-plus");
  ...
  $(document).on("click", ".fa-plus", function () {
    console.log("Click into PLUS"); // get .box parent
  ...
  $(".fa-plus").on("click", function (event) {// cannot use this because after removeClass the document NOT YET update, so dont know which element is
  }); /// jquery 2.2.1
